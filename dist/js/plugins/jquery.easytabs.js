...
(function ($) {
  $.easytabs = function (container, options) {
    ...
    plugin.init = function () {
      ...
      settings.bind_str = settings.event + ".easytabs"; // Add jQuery UI's crazy class names to markup,
    ...
    plugin.getTabs = function () {
      ...
      plugin.panels = $(), plugin.tabs.each(function () {
        ...
        $matchingPanel = settings.panelContext.find("#" + targetId); // If tab has a matching panel, add it to panels
        ...
        if ($matchingPanel.length) {
          ...
          $tab.data('easytabs').panel = $matchingPanel; // Otherwise, remove tab from tabs collection
        } else {
    ...
    plugin.selectTab = function ($clicked, callback) {
      var url = window.location,
          ...
          ajaxUrl = $clicked.parent().data('easytabs').ajax; // Tab is collapsible and active => toggle collapsed state

      if (settings.collapsible && !skipUpdateToHash && ($clicked.hasClass(settings.tabActiveClass) || $clicked.hasClass(settings.collapsedClass))) {
        plugin.toggleTabCollapse($clicked, $targetPanel, ajaxUrl, callback); // Tab is not active and panel is not active => select tab
      } else if (!$clicked.hasClass(settings.tabActiveClass) || !$targetPanel.hasClass(settings.panelActiveClass)) {
        activateTab($clicked, $targetPanel, ajaxUrl, callback); // Cache is disabled => reload (e.g reload an ajax tab).
    ...
    plugin.toggleTabCollapse = function ($clicked, $targetPanel, ajaxUrl, callback) {
      ...
      if (fire($container, "easytabs:before", [$clicked, $targetPanel, settings])) {
        plugin.tabs.filter("." + settings.tabActiveClass).removeClass(settings.tabActiveClass).children().removeClass(settings.tabActiveClass); // If panel is collapsed, uncollapse it
    ...
    var setDefaultTab = function setDefaultTab() {
      ...
      if ($selectedTab.length === 1) {
        ...
        if ($panel.length) {
          hash = '#' + $panel.attr('id');
          $defaultTab = plugin.matchTab(hash).parent(); // Otherwise, make the default tab the one that's active on page-load
        } else {
    ...
    var activateDefaultTab = function activateDefaultTab($selectedTab) {
    ...
    var activateTab = function activateTab($clicked, $targetPanel, ajaxUrl, callback) {
      plugin.panels.stop(true, true);
      ...
      if (fire($container, "easytabs:before", [$clicked, $targetPanel, settings])) {
        ...
        showPanel = function showPanel() {
          ...
          $container.trigger("easytabs:midTransition", [$clicked, $targetPanel, settings]); // Gracefully animate between panels of differing heights, start height change animation *after* panel change if panel needs to contract,
          ...
          $targetPanel[transitions.show](transitions.speed, settings.transitionInEasing, function () {
            ...
            $container.trigger("easytabs:after", [$clicked, $targetPanel, settings]); // callback only gets called if selectTab actually does something, since it's inside the if block
    ...
    var getHeightForHidden = function getHeightForHidden($targetPanel) {
      ...
      var display = $targetPanel.css('display'),
          outerCloak,
          height; // Workaround with wrapping height, because firefox returns wrong
    ...
    var setAndReturnHeight = function setAndReturnHeight($visiblePanel) {
